...
function page({ params }: Props) {
  ...
  return (
    <main className="p-8">
      <section className="py-8 px-[12rem] bg-[var(--color--bg-2)] rounded-lg">
        <div className="flex flex-col items-center">
          <h1 className="text-2xl font-bold text-[#6FCF97] !text-2xl !text-[#6FCF97]">
            {creatorDetails?.name}
          </h1>
        ...
        <div className="mt-12 flex justify-between gap-14">
          <div>
            <h3 className="text-lg font-bold !text-lg !font-bold">User Bio</h3>
            <p className="text-gray-300">{creatorDetails?.bio}</p>
          </div>
          <div className="px-16 py-8 bg-[var(--color--bg-1)] flex flex-col gap-6 rounded-lg text-lg text-gray-300">
            <Link
              target="_blank"
