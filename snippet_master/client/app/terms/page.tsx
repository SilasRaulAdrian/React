...
function page() {
  ...
  return (
    <div>
      ...
      <div className="flex">
        <HelpSidebar />
        <div className="px-8 py-10 flex-1 flex flex-col gap-4 bg-[var(--color--bg-2)]">
          {terms.map((section, index) => (
            <motion.div
              key={index}
              className={`py-4 px-6 border-[var(--color--rgba-3)] ${
                terms.length - 1 === index ? "border-b-none" : "border-b-[1px]"
              }`}
              variants={animateY}
              initial="hidden"
              animate="visible"
