...
function Categories() {
  ...
  return (
    <div className="fixed w-full z-10">
      <div className="pl-14 pr-[14rem] py-5 bg-[var(--color--bg-3)] border-b-2 border-[var(--color--rgba-2)]">
        <Carousel className="w-full lg:max-w-[1200px] xl:max-w-[1450px]">
          <CarouselContent className="flex gap-4">
            <CarouselItem
              className={`relative px-6 py-1 rounded-full cursor-pointer border-[0.1rem] border-[var(--color--rgba-1)] select-none
                ${
                  activeTag === "All"
            ...
            {tags.map((tag: ITag) => {
              return (
                <CarouselItem
                  key={tag._id}
                  className={`relative px-6 py-1 text-sm flex items-center rounded-full cursor-pointer border-[0.1rem] border-[var(--color--rgba-1)] select-none
                ${
                  activeTag === tag.name
                    ? "text-white bg-[#7263F3]"
                    : "bg-[#3A3B3C] text-white hover:text-gray-800 hover:bg-white transition-all duration-300 ease-in-out"
